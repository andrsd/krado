...
int
PolyMesh::split_triangle(double x,
                         double y,
                         double z,
                         Face * f,
...
intersect(PolyMesh::Vertex * v0,
          ...
          PolyMesh::Vertex * b1)
{
    double s0 = orient2d(v0->position, v1->position, b0->position);
    double s1 = orient2d(v0->position, v1->position, b1->position);
    if (s0 * s1 >= 0)
        return false;
    double t0 = orient2d(b0->position, b1->position, v0->position);
    double t1 = orient2d(b0->position, b1->position, v1->position);
    if (t0 * t1 >= 0)
        return false;
    return true;
}
...
PolyMesh::Face *
walk(PolyMesh::Face * f, UVParam uv)
{
...
recover_edge(PolyMesh & pm, PolyMesh::Vertex * v_start, PolyMesh::Vertex * v_end)
{
    ...
    auto n_intersections = list.size();
    while (!list.empty()) {
        he = *list.begin();
