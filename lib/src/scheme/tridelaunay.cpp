...
build_vertices(Ptr<MeshSurface> surface, const std::vector<Ptr<MeshCurve>> & curves)
{
...
bool
mesh_generation(Ptr<MeshSurface> surface,
                const std::vector<Ptr<MeshCurve>> & curves,
                bool only_initial_mesh)
{
    ...
    if (not boundary.empty()) {
        Log::error("The 1D mesh seems not to be forming a closed loop ({} boundary "
                   ...
                   boundary.size());
        // for (auto & bnd : boundary)
        //     Log::debug("Remaining node {}", bnd->num());
    ...
    // densified
    Log::debug("Recovering {} model edges", curves.size());
...
SchemeTriDelaunay::SchemeTriDelaunay(Options options) :
    Scheme2D(SchemeTriDelaunay::name),
    Scheme1D(SchemeTriDelaunay::name),
    opts_(options)
{
}

void
SchemeTriDelaunay::mesh_curve(Ptr<MeshCurve> /*mcurve*/)
{
}
...
void
SchemeTriDelaunay::mesh_surface(Ptr<MeshSurface> surface)
{
    Log::info("Meshing surface {}: scheme='{}'", surface->id(), SchemeTriDelaunay::name);
