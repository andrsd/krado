// SPDX-FileCopyrightText: 2026 David Andrs <andrsd@gmail.com>
// SPDX-License-Identifier: MIT

#include "krado/surface_index_mapper.h"
#include "krado/geom_surface.h"
#include "krado/mesh_surface.h"
#include "krado/mesh_curve.h"
#include "krado/mesh_vertex.h"
#include "krado/mesh_curve_vertex.h"
#include "krado/mesh_surface_vertex.h"
#include "krado/range.h"

namespace krado {

SurfaceIndexMapper::SurfaceIndexMapper(Ptr<MeshSurface> surface) : surface_(surface)
{
    for (auto & curve : surface->curves()) {
        for (auto & v : curve->bounding_vertices()) {
            auto pt = v->point();
            this->surf_idx_.try_emplace(pt, v);
        }
        for (auto & v : curve->curve_vertices()) {
            auto pt = v->point();
            this->surf_idx_.try_emplace(pt, v);
        }
    }

    for (auto cidx : make_range(surface->curves().size())) {
        auto curve = surface->curves()[cidx];
        for (auto & v : curve->bounding_vertices()) {
            auto pt = v->point();
            this->curv_surf_idx_[cidx].try_emplace(pt, v);
        }
        for (auto & v : curve->curve_vertices()) {
            auto pt = v->point();
            this->curv_surf_idx_[cidx].try_emplace(pt, v);
        }
    }
    for (auto & v : surface->surface_vertices()) {
        auto pt = v->point();
        this->surf_idx_.try_emplace(pt, v);
    }
}

SurfaceIndexMapper::~SurfaceIndexMapper() = default;

Ptr<MeshVertexAbstract>
SurfaceIndexMapper::surface_vertex(double x, double y)
{
    Point pt(x, y);
    auto it = this->surf_idx_.find(pt);
    if (it == this->surf_idx_.end()) {
        const auto & geom_surface = surface_->geom_surface();
        auto uv = geom_surface.parameter_from_point(pt);
        auto sv = Ptr<MeshSurfaceVertex>::alloc(geom_surface, uv);
        this->surf_idx_.try_emplace(pt, sv);
        this->surface_->add_vertex(sv);
        return sv;
    }
    else
        return it->second;
}

Ptr<MeshVertexAbstract>
SurfaceIndexMapper::curve_vertex(int curve_idx, double x, double y)
{
    auto & curv_map = this->curv_surf_idx_[curve_idx];
    Point pt(x, y);
    auto it = curv_map.find(pt);
    if (it == curv_map.end()) {
        auto curve = this->surface_->curves()[curve_idx];
        const auto & geom_curve = curve->geom_curve();
        auto u = geom_curve.parameter_from_point(pt);
        auto cv = Ptr<MeshCurveVertex>::alloc(geom_curve, u);
        this->curv_surf_idx_[curve_idx].try_emplace(pt, cv);
        curve->add_vertex(cv);

        this->surf_idx_.try_emplace(pt, cv);

        return cv;
    }
    else
        return it->second;
}

} // namespace krado
