...
/////////////////////////////////////////////////////////////////////////////////////
  ...
  class Edge {
   public:
    ...
    int Intersection(const Edge &e) const {
      if (!(adj[0] == &e || adj[1] == &e))
        std::cerr << "Bug : Intersection " << (void *)&e << "  " << adj[0] << " " << adj[1] << std::endl;
  ...
  class GeometricalVertex : public Vertex {
   public:
    ...
    void Set( ) { cas = 0; }
    void Set(const Vertex &v) {    // add for bug in clang++ compile FH very strang and hard  ...
      i = v.i;
      r = v.r;
      m = v.m;
      t = v.t;
  ...
  class Triangle {
   ...
   private:    // les arete sont opposes a un sommet
    Vertex
      *ns[3];    // 3 vertices if t is triangle, t[i] allowed by access function, (*t)[i] if pointer
  ...
  class ListofIntersectionTriangles {
   ...
   public:
    ...
    ListofIntersectionTriangles(int n = 256, int m = 16)
      : MaxSize(n), Size(0), len(-1), state(-1), lIntTria(new IntersectionTriangles[n]), NbSeg(0),
        MaxNbSeg(m), lSegsI(new SegInterpolation[m]) {
      if (verbosity > 9)
        std::cout << "      construct ListofIntersectionTriangles" << MaxSize << " " << MaxNbSeg << std::endl;
    };
    ...
    void NewSubSeg(GeometricalEdge *e, Real8 s0, Real8 s1) {
      if (NbSeg >= MaxNbSeg) {
        int mneo = MaxNbSeg;
        MaxNbSeg *= 2;
        if (verbosity > 3) std::cout << " reshape lSegsI from " << mneo << " to " << MaxNbSeg << std::endl;
    ...
    void ReShape( ) {
      ...
      for (int i = 0; i < MaxSize; i++)    // recopy
        nw[i] = lIntTria[i];
      if (verbosity > 3)
          std::cout << " ListofIntersectionTriangles  ReShape MaxSize " << MaxSize << " -> " << newsize
             << std::endl;
      MaxSize = newsize;
  ...
  class Triangles {
   public:
    ...
    Triangles(Int4 nbvx, Triangles &BT, int keepBackVertices = 1) : Gh(BT.Gh), BTh(BT) {
      try {
        GeomToTriangles1(nbvx, keepBackVertices);
      } catch (...) {
        this->clear( );
        throw;
      }
    ...
    GeometricalEdge *ProjectOnCurve(Edge &AB, Vertex &A, Vertex &B, Real8 theta, Vertex &R,
                                    VertexOnEdge &BR, VertexOnGeom &GR);
    ...
    void IntersectConsMetric(const double *s, const Int4 nbsol, const int *typsols,
                             ...
                             const Real8 anisomax, const Real8 CutOff = 1.e-4,
  ...
  inline double Triangle::QualityQuad(int a, int option) const {    // first do the logique part
    double q;
    if (!link || aa[a] & 4)
      q = -1;
    else {
      Triangle *t = at[a];
      if (t - this < 0)
        q = -1;    // because we do 2 times
      else if (!t->link)
        q = -1;
      else if (aa[0] & 16 || aa[1] & 16 || aa[2] & 16 || t->aa[0] & 16 || t->aa[1] & 16 ||
               t->aa[2] & 16)
        q = -1;
      else if (option) {
  ...
  inline void Vertex::Set(const Vertex &rec, const Triangles &, Triangles &) { *this = rec; }
  inline void GeometricalVertex::Set(const GeometricalVertex &rec, const Geometry &,
                                     const Geometry &) {
    *this = rec;
  }
  inline void Edge::Set(const Triangles &Th, Int4 i, Triangles &ThNew) {
    *this = Th.edges[i];
    ...
    if (on) on = ThNew.Gh.edges + Th.Gh.Number(on);
    if (adj[0]) adj[0] = ThNew.edges + Th.Number(adj[0]);
    if (adj[1]) adj[1] = ThNew.edges + Th.Number(adj[1]);
  }
  inline void GeometricalEdge::Set(const GeometricalEdge &rec, const Geometry &Gh,
                                   Geometry &GhNew) {
    *this = rec;
  ...
  inline void GeometricalSubDomain::Set(const GeometricalSubDomain &rec, const Geometry &Gh,
  ...
  inline void Triangles::UnMarkUnSwapTriangle( ) {
    Int4 i;
    for (i = 0; i < nbt; i++)
      for (int j = 0; j < 3; j++) triangles[i].SetUnMarkUnSwap(j);
  }
  ...
  inline void TriangleAdjacent::SetAdj2(const TriangleAdjacent &ta,
                                        int l) {    // set du triangle adjacent
    if (t) {
      t->at[a] = ta.t;
      t->aa[a] = ta.a | l;
    }
  ...
  inline int TriangleAdjacent::Locked( ) const { return t->aa[a] & 4; }
  inline int TriangleAdjacent::Cracked( ) const { return t->aa[a] & 32; }
  ...
  inline Real4 qualite(const Vertex &va, const Vertex &vb, const Vertex &vc) {
    ...
    if (deta <= 0)
      ...
      Real8 a = sqrt((Real8)(ac, ac)), b = sqrt((Real8)(bc, bc)), c = sqrt((Real8)(ab, ab)),
            p = a + b + c;
  ...
  int inline TriangleAdjacent::swap( ) { return t->swap(a); }
  ...
  void inline Triangle::checka(Int1 a) {
    ...
    assert(a2 < 3 && a2 >= 0);
    if (t2 &&
        (((*t1).ns[VerticesOfTriangularEdge[a1][0]] != (*t2).ns[VerticesOfTriangularEdge[a2][1]]) ||
      ...
      if (CurrentTh)
        cerr << "t1=" << t1 << " " << a1 << " "
             << CurrentTh->Number(t1->ns[VerticesOfTriangularEdge[a1][0]]) << " "
             << CurrentTh->Number(t1->ns[VerticesOfTriangularEdge[a1][1]]) << endl;
      if (t2)
        cerr << "t2=" << t2 << " " << a2 << " " << t2->ns[VerticesOfTriangularEdge[a2][0]] << " "
  ...
  void inline Triangle::check( ) {
    ...
    if (det >= 0)
      if (det != (det2 = bamg::det(*ns[0], *ns[1], *ns[2]))) {    // penthickness(4);Draw();
        if (CurrentTh) cerr << " In Triangles" << CurrentTh->Number(this) << endl;
        cerr << *ns[0] << *ns[1] << " " << *ns[2] << " " << endl;
        cerr << " Bug in triangle " << this << ":" << det << " !=  " << det2 << endl;
        MeshError(5);
      }
    checka(0);
    checka(1);
