...
namespace bamg {
                               ...
                               int *equiedges)    // construct a geometry if no geo
  {
    ...
    //
    // --------------------------
    if (verbosity > 1) cout << "  -- construction of the geometry from the 2d mesh " << endl;
    if (nbt <= 0 || nbv <= 0) {
      MeshError(101);
    }
    ...
    if (nbe != edge4->nb( )) {
      cerr << " Some Double edge in the mesh, the number is " << nbe << " nbe4=" << edge4->nb( )
           << endl;
      MeshError(1002);
    }
    for (i = 0; i < nbt; i++)
      for (j = 0; j < 3; j++) {
        ...
        if (st[k] == -1)
          ...
          cerr << " Edge " << triangles[(-st[k] + 2) / 3].NuEdgeTriangleAdj((int)((-st[k] + 2) % 3))
               << " Of Triangle "
               << Number(triangles[(-st[k] + 2) / 3].TriangleAdj((int)((-st[k] + 2) % 3))) << endl;
          MeshError(9999);
        }
      }
    ...
    for (i = 0; i < nbedges; i++)
      if (st[i] < -1)    // edge internal
      {
        ...
        // cout << it << " c="  << triangles[it].color <<  " " << Number(tt) << " c="  << tt.color
        // << endl;
        if (triangles[it].color != tt.color ||
            i < nbeold)    // Modif FH 06122055 // between 2 sub domai
          k++;
    ...
    if (k) {
      ...
      k = 0;
      // construction of the edges
      if (verbosity > 4) cout << "    Construction of the edges  " << nbe << endl;
      int kos = 0;
      for (i = 0; i < nbedges; i++) {
        Int4 add = -1;
        ...
        if (add >= 0 && add < nbe) {
          ...
          if (i < nbeold)    // in file edge // Modif FH 06122055
          {
            ...
            edges[add].ref =
              Min(edges[add].v[0]->ref( ), edges[add].v[1]->ref( ));    // no a good choice
        }
      }
      assert(k == nbe);
...

    {
        ...
        if (colorT[it] < 0) {
          ...
          while (level > 0)
            if ((j = st[level]++) < 3) {
              ...
              level -= 2;
          if (verbosity > 5)
            cout << "   Nb of triangles " << k << " of Subdomain " << NbSubDomains << " " << kolor
                 << endl;
          NbSubDomains++;
        }
      }
      if (verbosity > 3) cout << "   The Number of sub domain = " << NbSubDomains << endl;
      ...
      for (it = 0; it < nbt; it++)
        for (int j = 0; j < 3; j++) {
          ...
          if ((!tt || triangles[it].Locked(j) || tt->color != triangles[it].color) &&
              !subdomains[isd = colorT[it]].head) {
    ...
    if (verbosity > 6) {
      ...
      for (i = 0; i < Gh.nbv; i++)
        if (Gh.vertices[i].Required( )) nbr++;
      cout << " --bb nb  require  v in Gh " << nbr << " on " << Gh.nbv << endl;
    }
    ...
    if (verbosity > 6) {
      ...
      for (i = 0; i < Gh.nbv; i++)
        if (Gh.vertices[i].Required( )) nbr++;
      cout << " --11 nb  require  v in Gh " << nbr << " on " << Gh.nbv << " kreq =" << kreq
           << " kkreq=" << kkreq << endl;
    }
  ...
  Geometry::Geometry(const Geometry &Gh) {
    ...
    *this = Gh;
    ...
    for (i = 0; i < nbv; i++) vertices[i].Set(Gh.vertices[i], Gh, *this);
    for (i = 0; i < nbe; i++) edges[i].Set(Gh.edges[i], Gh, *this);
    for (i = 0; i < NbOfCurves; i++) curves[i].Set(Gh.curves[i], Gh, *this);
    for (i = 0; i < NbSubDomains; i++) subdomains[i].Set(Gh.subdomains[i], Gh, *this);
...
#define MXE__LINE __LINE__ + 1
    ...
    if (NbTry)
      cout << " G edge= " << Ag << Bg << endl
           << " v edge" << V01 << " v geom " << AB << (V01, AB) << endl;
    ...
    if ((cos0A < 0 && cos0B < 0) || (cos1A > 0 && cos1B > 0)) {
      cerr << "  Bug on pointer edge  [" << V0 << " , " << V1 << " ] "
           << "  on geometrical edge " << Number(on) << " = [" << Ag << " , " << Bg << " ] "
           << endl;
    ...
    while (eg0 != (GeometricalEdge *)vg0 && (*eg0)(sens0) != (GeometricalVertex *)vg0) {
      if (bge <= 0) {
        ...
        if (NbTry) {
          cerr << " -- Fatal Error: on the class triangles before call Geometry::ProjectOnCurve"
               << endl;
          cerr << "   The mesh of the  Geometry is to fine: ";
          cerr << "     1)  a mesh edge  contening more than " << mxe / 2 << " geometrical edges."
               << endl;
          cerr << "     2)  code bug : be sure that we call   Triangles::SetVertexFieldOn() before "
               << endl;
          cerr << "   To solve the problem do a coarsening of the geometrical mesh " << endl;
          cerr << " or change the constant value of mxe in " << __FILE__ << " line " << MXE__LINE
               << "( dangerous way )" << endl;
          MeshError(222);
        }
        NbTry++;
      ...
      if (NbTry)
        cout << "bug: --Edge @" << Number(tmpge) << " = " << Number(eg0) << ":"
             << Number(eg0->Adj[0]) << "," << Number(eg0->Adj[1]) << ",";
      ...
      if (NbTry)
        cout << "bug: Edge " << Number(eg0) << " " << 1 - sens0 << " S "
             ...
             << Number(eg0) << (*eg0)[sens0].r << "v = " << Number((*eg1)(sens0)) << " e = " << eg0
             << endl;
    }
    if (NbTry)
    ...
    while (eg1 != (GeometricalEdge *)vg1 && (*eg1)(sens1) != (GeometricalVertex *)vg1) {
      ...
      if (NbTry)
        cout << "  Edge " << Number(eg1) << " " << sens1 << " S " << Number((*eg1)[sens1]) << "%"
             << Number(eg1->Adj[0]) << "," << Number(eg1->Adj[1]) << "," << Number(eg1)
             << (*eg1)[sens1].r << "v = " << Number((*eg1)(sens1)) << " e = " << Number(eg1)
             << endl;
    }

    if (NbTry) cout << endl;

    if ((*eg0)(sens0) == (GeometricalVertex *)vg0) vg0 = VertexOnGeom(*(Vertex *)vg0, *eg0, sens0);

    if ((*eg1)(sens1) == (GeometricalVertex *)vg1) vg1 = VertexOnGeom(*(Vertex *)vg1, *eg1, sens1);
  ...
  void Geometry::AfterRead( ) {    // -----------------
    if (verbosity > 20) cout << "Geometry::AfterRead()" << nbv << " " << nbe << endl;
    Int4 i, k = 0;
    ;
    int jj;    // jj in [0,1]
    ...
    float *eangle = new float[nbe];
    {
      ...
      for (i = 0; i < nbv; i++) {
        ...
        if (v && Norme1(v->r - vertices[i]) < eps) {    // link v & vertices[i]
    ...
    for (i = 0; i < nbe; i++) {
      ...
      for (jj = 0; jj < 2; jj++) {    // generation of list
    ...
    for (i = 0; i < nbv; i++) {
      ...
      if (ord == 2) {    // angulare test to find a corner
        ...
        if ((da12 >= MaximalAngleOfCorner) && (da12 <= 2 * Pi - MaximalAngleOfCorner)) {
          ...
          if (verbosity > 7)
            cout << "     The vertex " << i << " is a corner (angle) " << 180 * (da12) / Pi << " "
                 << 180 * MaximalAngleOfCorner / Pi << endl;
        }
        ...
        if (edges[i1].flag != edges[i2].flag || edges[i1].Required( )) {
          ...
          if (verbosity > 7)
            cout << "     The vertex " << i
                 << " is Required the flag change (crack or equi, or require)" << endl;
        }

        if (edges[i1].ref != edges[i2].ref) {
          vertices[i].SetRequired( );
          if (verbosity > 7) cout << "     The vertex " << i << " is Required ref" << endl;
        }
      };
    ...
    for (i = 0; i < nbe; i++)
      for (jj = 0; jj < 2; jj++) {
        ...
        if (edges[i1].v[j1] != edges[i].v[jj]) {
          cerr << " Bug Adj edge " << i << " " << jj << " et " << i1 << " " << j1 << " k=" << k;
      ...
      for (jj = 0; jj < 2; jj++) {
        ...
        if (ltg == 0) {                        // no tg
          if (!edges[i].v[jj]->Corner( )) {    // not a Corner
            tg = edges[i].v[1 - jj]->r - edges[i].Adj[jj]->v[1 - edges[i].SensAdj[jj]]->r;
            ltg = Norme2(tg);
            tg = tg * (lAB / ltg), ltg = lAB;
            /*
               if(edges[i].ref >=4)
               cout << " tg " << tg.x << " "<< tg.y  << " " << edges[i].v[1-jj]->r <<
               edges[i].Adj[jj]->v[1-edges[i].SensAdj[jj]]->r << " y-y = "
    ...
    for (int step = 0; step < 2; step++) {
      ...
      for (int level = 0; level < 2 && nbgem != nbe; level++)
        for (jj = 0; jj < 2;
             jj++)    //  change the sens of loop to keep interior normal in general..  feb. 2017
          for (i = 0; i < nbe; i++)
    ...
    for (int i = 0; i < NbOfCurves; i++) {
      ...
      if (be->Equi( ) || be->ReverseEqui( )) {
        ...
        if (verbosity > 9) {
          ...
          cout << "    curve  " << i << ": " << Number(be) << " <=> " << Number(eqbe) << " "
               << be->Equi( ) << be->ReverseEqui( ) << " -->  " << Number(ee) << " <=>"
               << Number(eqee) << " " << ee->Equi( ) << ee->ReverseEqui( ) << endl;
          cout << "    curve eq: " << nc << ": " << Number(bee) << " " << bee->Equi( )
               << bee->ReverseEqui( ) << " -->  " << Number(eee) << " !" << eee->Equi( )
               << eee->ReverseEqui( ) << endl;
        }
      }
    }
  ...
  R2 GeometricalEdge::F(Real8 theta) const    // parametrization of the curve edge
  {
    ...
    if (TgA( ))
      if (TgB( ))    // interpolation d'hermite
      {
        ...
        ctb = (theta - 1) * theta * theta;
        //  if(ref==4 || ref==5)
        //  cout << " FFF " << tg[0] << tg[1] << A << B << " => " << A*ca + B*cb + tg[0]* cta +
        //  tg[1] * ctb << endl;
      } else {    // 1-t*t, t-t*t, t*t
        Real8 t = theta;
        cb = t * t;
