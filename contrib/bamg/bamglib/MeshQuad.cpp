...
//#pragma inline_depth 0
  ...
  template< class T >
  inline void HeapSort(T *c, long n) {
    long l, j, r, i;
    T crit;
  ...
  double QuadQuality(const Vertex &a, const Vertex &b, const Vertex &c, const Vertex &d) {
    ...
    // cout << A << B << C << D ;
    // cout << " sinmin " << sinmin << " " << cosDAB << " " << cosABC << " " << cosBCD << " " <<
    // cosCDA << endl; rattente(1);
    if (sinmin <= 0) return sinmin;
    return 1.0 - Max(Max(Abs(cosDAB), Abs(cosABC)), Max(Abs(cosBCD), Abs(cosCDA)));
  }

  GeometricalEdge *Triangles::ProjectOnCurve(Edge &BhAB, Vertex &vA, Vertex &vB, Real8 theta,
...

  {
    ...
    if (vA.vint == IsVertexOnEdge) {    // find the start edge
      e = vA.onbe->be;
    ...
    for (int cas = 0; cas < 2; cas++) {    // 2 times algo:
      ...
      for (eee = e, iii = sens, te0 = tA;
           ...
           neee = eee->adj[iii], iii = 1 - neee->Intersection(*eee), eee = neee, v0 = v1,
          te0 = 1 - iii) {
      ...
      if (v1 != pvB) {
        ...
        abscisse = lg * theta;
        if (abscisse <= lg &&
            abscisse >= lg0)    // small optimisation we know the lenght because end
  ...
  void Triangles::MakeQuadrangles(double costheta) {
    if (verbosity > 2) cout << "  -- MakeQuadrangles costheta = " << costheta << endl;
    if (verbosity > 5)
    ...
    if (costheta > 1) {
      if (verbosity > 5) cout << "     do nothing costheta >1 " << endl;
      return;
    }
    ...
    for (i = 0; i < nbt; i++)
      for (j = 0; j < 3; j++)
        if ((qq[k].q = triangles[i].QualityQuad(j)) >= costheta) qq[k++].i3j = i * 3 + j;
    //  sort  qq
    HeapSort(qq, k);
    ...
    for (ij = 0; ij < k; ij++) {
      ...
      if (triangles[i].QualityQuad(j, 0) >= costheta) triangles[i].SetHidden(j), kk++;
    }
    NbOfQuad = kk;
    if (verbosity > 2) {
  ...
  */
  int Triangles::SplitElement(int choice) {
    ...
    if (verbosity > 5)
      cout << endl
           << "    (in)  Nb of Quadrilaterals = " << NbOfQuad
           << " Nb Of Triangles = " << nbt - NbOutT - NbOfQuad * 2
           << " Nb of outside triangles = " << NbOutT << endl;
...
#ifdef DEBUG
    for (i = 0; i < nbt; i++) triangles[i].check( );
#endif
#ifdef DRAWING2
    reffecran( );
#endif
    ...
    for (i = 0; i < nbe; i++) {
      ...
      if (ong)    // a geometrical edges
      {
        if (withBackground) {
          ...
          assert(edgesGtoB);
          // cout << " ie = " << ie <<"  v0 = " <<  Number(newedges[ie][0]) << endl;
          ong = ProjectOnCurve(*edgesGtoB[Gh.Number(edges[i].on)], edges[i][0], edges[i][1], 0.5,
                               vertices[k], newVertexOnBThEdge[kvb], newVerticesOnGeomEdge[kvg++]);
        ...
        if (ke > 0) {
          ...
          if ((dd = Area2(v0.r, v1.r, A.r)) >= 0) {    // warning PB roundoff error
            if (t.link && ((aa = Area2(A.r, t[1].r, t[2].r)) < 0.0 ||
              ...
              ferr++, cerr << " Error : " << ke + nbvold << " not in triangle " << i
                           << " In=" << !!t.link << " " << aa << " " << bb << " " << cc << " " << dd
                           << endl;
    ...
    //
    if (withBackground)
      for (i = 0; i < BTh.nbv; i++) BTh.vertices[i].m = BTh.vertices[i].m * 2.;
#ifdef DRAWING2
    Draw( );
    inquire( );
#endif
    ...
    if (withBackground) BTh.ReMakeTriangleContainingTheVertex( );
