...
#define J_IJ(k, l) ((k & 2) ? l : 0)
      ...
      while (pi[l]--) {
        ...
        } else    // Pointer QuadTreeBox
        {
          ...
          if ((b = b->b[k])) {
            ...
            if (INTER_SEG(iii, iii + hb, iplus - h, iplus + h) &&
                INTER_SEG(jjj, jjj + hb, jplus - h, jplus + h)) {
              pb[++l] = b;
      ...
      while (pi[l]--) {
        ...
        } else    // Pointer QuadTreeBox
        {
          ...
          if ((b = b->b[k])) {
            ...
            if (INTER_SEG(iii, iii + hb, i - hx, i + hx) &&
                INTER_SEG(jjj, jjj + hb, j - hy, j + hy)) {
              pb[++l] = b;
              pi[l] = b->n > 0 ? (int)b->n : 4;
              ii[l] = iii;
              jj[l] = jjj;
  ...
  void QuadTree::Add(Vertex &w) {
    ...
    while ((b = *pb) && (b->n == 4))    // the QuadTreeBox is full
    {
      ...
      for (int k = 0; k < 4; k++)    // for the 4 vertices find the sub QuadTreeBox ij
      {
        ...
        if (!bb) bb = b->b[ij] = NewQuadTreeBox( );    // alloc the QuadTreeBox
        //    cout << bb << " " << k << " "  << ij <<  endl;
        bb->v[bb->n++] = v4[k];
      }
      pb = &b->b[IJ(i, j, l)];
  ...
  QuadTree::QuadTree(Triangles *t, long nbv)
    : lenStorageQuadTreeBox(t->nbvx / 8 + 10), th(t), NbQuadTreeBox(0), NbVertices(0),
      NbQuadTreeBoxSearch(0), NbVerticesSearch(0) {
    if (nbv == -1) nbv = t->nbv;
    sb = new StorageQuadTreeBox(lenStorageQuadTreeBox);
    root = NewQuadTreeBox( );
    assert(MaxISize > MaxICoor);
    for (Int4 i = 0; i < nbv; i++) Add(t->vertices[i]);
#ifdef DRAWING1
    Draw( );
#endif
  }
  ...
  ostream &operator<<(ostream &f, const QuadTree &qt) {
    ...
    f << " NbQuadTreeBoxSearch " << qt.NbQuadTreeBoxSearch << " NbVerticesSearch "
      << qt.NbVerticesSearch << endl;
