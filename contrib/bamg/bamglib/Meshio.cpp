...
namespace bamg {
  ...
  ///////////////////////////////////////////////////////////
  double *ReadBBFile(const char *file, long &nbsol, long &lsol, int *&typesols, const int dim,
                     const int typesol) {
  ...
  void MeshIstream::ShowIoErr(int s) {
    ...
    cerr << ", Erreur Lecture " << s << ", good=" << (s & ios::goodbit)
         << ", bad=" << (s & ios::badbit) << ", fail=" << (s & ios::failbit)
  ...
  int MeshIstream::IsString(const char *s) {
    ...
    if (in.good( ))
      while (*ss && c == *ss && in.get(c)) ss++;

    if (*ss) {    // no
      if (verbosity > 9)
        cout << "IsString: not " << s << " " << n << "  putback " << ss - s << " :" << c;
      if (in.good( )) in.putback(c), ss--;
      while (ss - s > 0) {
        cout << *ss;
        in.putback(*ss--);
      };
  ...
  OFortranUnFormattedFile::~OFortranUnFormattedFile( ) {
    ...
    if (l == 0) {    // l rec no set --
      l = where( );
      if (verbosity > 9)
        cout << " size of last record  = " << l << " n = " << nb_rec << " i= " << i << endl;
      if (!f->good( )) Error(3);
  ...
  long IFortranUnFormattedFile::Record( ) {
    ...
    n += l + sizeof(l);    // end
    if (verbosity > 9)
      cout << " Read  rec end =" << n << " l=  " << l << " current= " << j << " begin= " << i
           << endl;
    return l;
  }
  ...
  long OFortranUnFormattedFile::Record(long ll)    //
  {
    ...
    if (n >= 0) {
      if (l == 0) {    // l rec no set --
        l = where( );
        if (verbosity > 9)
          cout << " set len  of rec  " << nb_rec << " = " << l << " @ " << i - sizeof(long) << endl;
    ...
    n += l;
    if (verbosity > 9)
      cout << " Write rec end =" << n << " l=  " << l << " current= " << j << " begin= " << i
           << endl;
    return l;
  }
  void OFortranUnFormattedFile::Error(int err) {
    ...
    cerr << " position in the record = " << where( ) << " length of record = " << l << endl;
