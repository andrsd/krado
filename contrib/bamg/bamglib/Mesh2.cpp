...
#define srandom init_genrand
  ...
  Int4 AGoodNumberPrimeWith(Int4 n) {
    const Int4 BigPrimeNumber[] = {567890359L, 567890431L, 567890437L, 567890461L, 567890471L,
    ...
    for (int i = 0; BigPrimeNumber[i]; i++) {
      ...
      if (o < oo) o = oo, pi = BigPrimeNumber[i];
    }
  ...
  void swap(Triangle *t1, Int1 a1, Triangle *t2, Int1 a2, Vertex *s1, Vertex *s2, Icoor2 det1,
    ...
    TriangleAdjacent taas1 = t1->Adj(as1), taas2 = t2->Adj(as2), tas1(t1, as1), tas2(t2, as2),
                     ta1(t1, a1), ta2(t2, a2);
#ifdef DEBUG
    assert(!ta1.Locked( ));
    assert(!ta2.Locked( ));
  ...
  TriangleAdjacent Triangle::FindBoundaryEdge(int i) const {
    ...
    do {
      ...
      ttc = t->at[j];
      exterieur = !ttc->link;
      if (exterieur + exterieurp == 1) return TriangleAdjacent(t, j);
  ...
  TriangleAdjacent CloseBoundaryEdgeV2(I2 C, Triangle *t, double &a, double &b) {
    ...
    if (s) {
      ...
      do {    // loop around vertex s
        ...
        //	      <<  endl;
        if ((link + linkp) == 1) {    // a boundary edge
  ...
  void ListofIntersectionTriangles::SplitEdge(const Triangles &Bh, const R2 &A, const R2 &B,
    ...
    if (nbegin) {    // optimisation
      ...
      if (t->det >= 0)
        ilast =
          NewItem(t, Real8(deta[0]) / t->det, Real8(deta[1]) / t->det, Real8(deta[2]) / t->det);
      else {    // find the nearest boundary edge  of the vertex A
    ...
    for (;;) {
      ...
      if ((dt[i = VerticesOfTriangularEdge[i0][0]] < 0) &&
        ...
        if (k == 1) {
          ...
          cerr << " dt[0]= " << dt[0] << " dt[1]= " << dt[1] << " dt[2]= " << dt[2] << endl;
          cerr << i0 << " " << i1 << " " << i2 << endl;
          cerr << " A = " << A << " B= " << B << endl;
  ...
  Real8 ListofIntersectionTriangles::Length( ) {
    ...
    for (jj = 0, ii = 1; ii < Size; jj = ii++) {
      ...
      s += sxy;
      lIntTria[ii].s = s;
      if (ii == EndSeg) SegI->lEnd = s, SegI++, EndSeg = SegI->last, SegI->lBegin = s;
  ...
  Int4 ListofIntersectionTriangles::NewPoints(Vertex *vertices, Int4 &nbv, Int4 nbvx) {
    ...
    for (int k = 1; k < nbi; k++) {
      while ((ii < Size) && (lIntTria[ii].s <= si))
        if (ii++ == EndSeg) SegI++, EndSeg = SegI->last;
      ...
      if (nbv < nbvx) {
        ...
        if (k > 1) {
          ...
          if (dp > 1.6) {
            cerr << "PB calcul new Int.  points trop loin l=" << dp << " v=" << nbv - 1 << " "
                 << nbv - 2 << Mx << My << y - x << endl;
          }
        }
#endif
      } else
  ...
  int SwapForForcingEdge(Vertex *&pva, Vertex *&pvb, TriangleAdjacent &tt1, Icoor2 &dets1,
    ...
    if (CurrentTh &&
        !(((detvasasb <= 0) && (detvbsasb >= 0)) || ((detvasasb >= 0) && (detvbsasb <= 0)))) {
      cout << " detvasasb =" << detvasasb << "detvbsasb = " << detvbsasb << " " << pva << " " << pvb
           << " " << CurrentTh << endl;
#ifdef DRAWING1
      reffecran( );
      CurrentTh->Draw( );
    ...
    if ((ndet1 > 0) && (ndet2 > 0)) {    // on peut swaper
...
#ifdef DEBUG
    if (ForDebugging) {
      cerr << "swap = " << ToSwap << " ndet1 " << ndet1 << ", ndet2 " << ndet2 << "det1  " << det1
           << " det2 " << det2 << " if1 = " << ((ndet1 > 0) && (ndet2 > 0))
...
#endif
    if (ToSwap) NbSwap++, bamg::swap(t1, a1, t2, a2, &s1, &s2, ndet1, ndet2);
      ...
      if ((det1 < 0) && (det2 > 0)) {
        ...
        while ((ks = SwapForForcingEdge(va, vb, tc, detss, det1, det2, NbSwap)))
          if (l++ > 10000000) {
            ...
            if (CurrentTh)
              cerr << " vertex number " << CurrentTh->Number(a) << " " << CurrentTh->Number(b)
                   << endl;
#ifdef DEBUG
            ForDebugging = 1;
#endif
            ...
            if (CurrentTh) {
              ...
              while (ks = SwapForForcingEdge(va, vb, tc, detss, det1, det2, NbSwap) && (l++ < 1000))
                cerr << " " << CurrentTh->Number(tc.EdgeVertex(0)) << " "
                     << CurrentTh->Number(tc.EdgeVertex(1)) << " ";
            }
#endif
            MeshError(990);
          }
        ...
        if (((aa == &a) && (bb == &b)) || ((bb == &a) && (aa == &b))) {
          ...
          return -2;    // error  boundary is crossing
                        /*	  cerr << "Fatal Error  boundary is crossing  ";
                                  if(CurrentTh)
                                  {
                                      cerr << " edge:  [" << CurrentTh->Number(a) << ", " << CurrentTh->Number(b)
                           <<  " ] and [ ";               cerr    << CurrentTh->Number(aa) << " " << CurrentTh->Number(bb) << "
                           ] " << endl;
                                  }
                                  MeshError(991);
                        */
  ...
  int Triangle::swap(Int2 a, int koption) {
    ...
    if (sa == 0) {    // les deux triangles sont frontieres
      ...
      if (verbosity > 99999)
        cout << " Triangle::swap int " << OnSwap << " " << det1 << " " << det2 << " " << detMinNew
             << endl;
        ...
        if (!OnSwap) {
          ...
          while (1)
            if (kopt) {
              ...
              Icoor2 xb1 = sb->i.x - s1->i.x, x21 = s2->i.x - s1->i.x, yb1 = sb->i.y - s1->i.y,
                     y21 = s2->i.y - s1->i.y, xba = sb->i.x - sa->i.x, x2a = s2->i.x - sa->i.x,
                     yba = sb->i.y - sa->i.y, y2a = s2->i.y - sa->i.y;
              double cosb12 = double(xb1 * x21 + yb1 * y21), cosba2 = double(xba * x2a + yba * y2a),
                     sinb12 = double(det2), sinba2 = double(t2->det);

              // angle b12 > angle ba2 => cotg(angle b12) < cotg(angle ba2)
              OnSwap = ((double)cosb12 * (double)sinba2) < ((double)cosba2 * (double)sinb12);
              if (verbosity > 99999)
                cout << " Triangle::swap " << OnSwap << " " << ((double)cosb12 * (double)sinba2)
                     << " " << ((double)cosba2 * (double)sinb12) << " "
                     << ((double)cosba2 * (double)sinb12) - ((double)cosb12 * (double)sinba2)
                     << endl;
              //  	 if(CurrentTh)
    ...
    if (OnSwap && (munswap1 || munswap2)) {
      cout << " Error Mark unswap T " << CurrentTh->Number(t1) << " " << CurrentTh->Number(t2)
           << endl
           << *t1 << endl
           << *t2 << endl;
      return 0;
  ...
  void Triangles::Add(Vertex &s, Triangle *t, Icoor2 *det3) {
    ...
    if (((infv < 0) && (detOld < 0)) || ((infv >= 0) && (detOld > 0))) {
      ...
      cerr << Number(s) << " " << Number(s0) << " " << Number(s1) << " " << Number(s2) << endl;
      MeshError(3);
    }
...
#ifdef DEBUG1
        cout << " The point " << Number(s) << " is the edge " << izerodet << " of " << Number(t)
             << " det3 = " << det3[0] << " " << det3[1] << " " << det3[2] << " " << endl;
        cout << " ta = " << ta << "ta->det =" << ((Triangle *)ta)->det << " " << t->det << endl;
#endif
    ...
    *tt[1] = *tt[2] = *t;
    ...
    if (izerodet >= 0) {
      ...
      if (!rswap) {
        cout << " Problem swap: the point s is on an edge => swap " << iedge << " " << *tt[izerodet]
             << endl;
#ifdef DRAWING
  ...
  Int4 Triangles::SplitInternalEdgeWithBorderVertices( ) {
      ...
      if (t.link)
        for (int j = 0; j < 3; j++)
          if (!t.Locked(j) && !t.Hidden(j)) {
            ...
            Triangle &tt = *ptt;
            if (ptt && tt.link && it < Number(tt)) {    // an internal edge
              ...
              if (v0.on && v1.on) {
                ...
                NbSplitEdge++;
                if (verbosity > 7)
                  cout << " Internal edge with two vertices on boundary" << Number(v0) << " "
                       << Number(v1) << " by " << endl;
              }
            }
          }
    ...
    if (verbosity > 2)
      cout << "SplitInternalEdgeWithBorderVertices: Number of splited edges " << NbSplitEdge
           << endl;
    return NbSplitEdge;
  }
  Int4 Triangles::InsertNewPoints(Int4 nbvold, Int4 &NbTSwap) {
    ...
    if (verbosity > 5) cout << " Try to insert the " << nbvnew << " new points " << endl;
    if (verbosity > 99) cout << "       coefIcoor " << coefIcoor << " pmin:  " << pmin << endl;
    ...
    if (nbvnew) {
      ...
      if (verbosity > 3) {
        cout << "    Number of new points " << iv << " Number of points which are too close "
             << nbv - iv;
    ...
    if (verbosity > 3) cout << " NbSwap = " << NbSwap << endl;
...
#ifdef DEBUG
    {
      ...
      for (i = 0; i < nbt; i++)
        if (triangles[i].link) {
          double dd =
            Det(triangles[i][1].r - triangles[i][0].r, triangles[i][2].r - triangles[i][0].r);
          if (dd <= 0) {
            NbErr++;
            ...
            cerr << "I2     " << triangles[i][0].r << triangles[i][1].r << triangles[i][2].r
                 << endl;
            cerr << "R2     " << triangles[i][0].i << triangles[i][1].i << triangles[i][2].i
                 << endl;
      ...
      if (NbErr) {
        ...
        if (verbosity > 3) cout << " Strange number of swaps " << kkk << endl;
        if (kkk) {
          for (i = 0; i < nbt; i++)
            if (triangles[i].link) {
              double dd =
  ...
  void Triangles::NewPoints(Triangles &Bh, int KeepBackVertex) {    // Triangles::NewPoints
    Int4 nbtold(nbt), nbvold(nbv);
    if (verbosity > 2) cout << "  -- Triangles::NewPoints ";
    ...
    if (KeepBackVertex && (&Bh != this) && (nbv + Bh.nbv < nbvx)) {
      ...
      if (verbosity > 2)
        cout << "      (Nb of Points from background mesh  = " << nbv - nbvold << " / "
             << nbv1 - nbvold << ")" << endl;
    } else
      Bh.ReMakeTriangleContainingTheVertex( );
    ...
    do {
      ...
      for (t = &triangles[i]; i < nbt;
           t = &triangles[i = next_t], next_t = -first_np_or_next_t[i]) {    // for each triangle  t
      ...
      for (i = nbvold; i < nbv; i++) {    // for all the triangle contening the vertex i
        Vertex *s = vertices + i;
        ...
        do {
          ...
          if (first_np_or_next_t[kt] > 0) first_np_or_next_t[kt] = -Headt, Headt = kt;
  ...
  void Triangles::NewPointsOld(Triangles &Bh) {    // Triangles::NewPointsOld
    ...
    do {
      ...
      i = Headt;
      next_t = -first_np_or_next_t[i];
      for (t = &triangles[i]; i < nbt; t = &triangles[i = next_t],
          next_t = -first_np_or_next_t[i]) {    // for each triangle  t
                                                // we can change first_np_or_next_t[i]
#ifdef TRACETRIANGLE
        ...
        for (j = 0; j < 3; j++) {       // for each edge
          ...
          if (trace) {
            cout << "Test edge " << i << " AB = " << A << B << "i " << Number(vA) << "j"
                 << Number(vB);
            cout << " link" << (int)t->link << " ta=" << Number(ta) << " det " << ta->det;
        ...
        if (trace) {
          ...
          cout << "\n ------------ " << t->link << " " << t->det << " b " << BeginNewPoint[0] << " "
               << BeginNewPoint[1] << " " << BeginNewPoint[2] << " "
               << " e " << EndNewPoint[0] << " " << EndNewPoint[1] << " " << EndNewPoint[2] << " "
...
#ifdef DEBUG2
              cout << "ii1 = " << ii1 << " ii0 = " << ii0 << endl;
              cout << "   cccc = " << cii0 << " " << ciii << " " << cii1 << " sss=" << sss << endl;
#endif
              if (kstack > 5)    // bug ?
      ...
      for (i = nbtold; i < nbt; i++) first_np_or_next_t[i] = 1;
  ...
  void Triangles::Insert( ) {
    ...
    const Int4 PrimeNumber = AGoodNumberPrimeWith(nbv);
    Int4 k3 = myrand( ) % nbv;
    for (int is3 = 0; is3 < nbv; is3++) ordre[is3] = &vertices[k3 = (k3 + PrimeNumber) % nbv];
    ...
    for (Int4 icount = 2; icount < nbv; icount++) {
      ...
      if (verbosity > 9999) cout << " Insert " << Number(vi) << ": " << *vi << endl;
    ...
    k3 = myrand( ) % nbv;
    for (int is4 = 0; is4 < nbv; is4++) ordre[is4] = &vertices[k3 = (k3 + PrimeNumber) % nbv];

    double timeloop = time2;
    for (int Nbloop = 0; Nbloop < NBLOOPOPTIM; Nbloop++) {
      double time000 = timeloop;
      Int4 NbSwap = 0;
      for (int is1 = 0; is1 < nbv; is1++) NbSwap += ordre[is1]->Optim(0, 0);
      timeloop = CPUtime( );
      if (verbosity > 3)
        cout << "    Optim Loop " << Nbloop << " NbSwap: " << NbSwap << " NbSwap/Nbv "
             << (float)NbSwap / (float)nbv << " CPU=" << timeloop - time000 << "  s, "
    ...
    time3 = CPUtime( );
    if (verbosity > 4)
      cout << "    init " << time1 - time0 << " initialisation,  " << time2 - time1
           << "s, insert point  " << time3 - time2 << "s, optim " << endl
  ...
  void Triangles::ForceBoundary( ) {
    if (verbosity > 2) cout << "  -- ForceBoundary: number of edges " << nbe << endl;
    int k = 0;
    Int4 nbfe = 0, nbswp = 0, Nbswap = 0;
    for (Int4 t = 0; t < nbt; t++)
      if (!triangles[t].det) k++, cerr << " det T " << t << " = " << 0 << endl;
    if (k != 0) {
    ...
    for (Int4 i = 0; i < nbe; i++) {
      ...
      if (nbswp < 0 && k < 5) {
        cerr << " Missing edge " << i << " v0 =  " << Number(edges[i][0]) << edges[i][0].r
             << " v1= " << Number(edges[i][1]) << edges[i][1].r << " " << edges[i].on->Cracked( )
             << " " << (Triangle *)ta;
        if (ta.t) {
      ...
      if (nbswp >= 0 && edges[i].on->Cracked( )) ta.SetCracked( );
    }
    ...
    for (Int4 j = 0; j < nbv; j++) Nbswap += vertices[j].Optim(1, 0);
    if (verbosity > 3)
  ...
  void Triangles::FindSubDomain(int OutSide = 0) {
    ...
    if (verbosity > 2) {
      ...
      if (verbosity > 99) {
        ...
        for (int i = 0; i < nbt; ++i) cout << i << " " << triangles[i] << endl;
      }
    }
    ...
    if (OutSide || !Gh.subdomains || !Gh.NbSubDomains) {    // No geom sub domain
      Int4 i;
      ...
      for (it = 0; it < nbt; it++) mark[it] = triangles[it].link ? -1 : -2;
      ...
      if (OutSide) {
          ...
          for (it = 0; it < nbt && nbk; it++)
            for (int na = 0; na < 3 && nbk; na++) {
              ...
              if (kr != kl) {
                ...
                if (kr < 0 && kl >= 0 && subdomains[kl].ref >= 0) nbk--, subdomains[kl].ref = -1;
                if (kl < 0 && kr >= 0 && subdomains[kr].ref >= 0) nbk--, subdomains[kr].ref = -1;
                //   cout << " after \t "
        ...
        for (i = 0; i < NbSubDomains; i++)
          if ((-subdomains[i].ref) %
            ...
            if (i != j) Exchange(subdomains[i], subdomains[j]);
            j++;
          } else {    // cout << " remove sub domain " << i << endl;
            t = subdomains[i].head;
            while (t) {    // cout << Number(t) << " " << endl;
              NbOutT++;
              t1 = t;
      ...
      for (Int4 i = 0; i < NbSubDomains; i++) {
        ...
        //	cout << " geom edge = " <<  Gh.Number(eg) <<" @" << &eg << " ref = " <<
        //subdomains[i].ref
        //     << " ref edge =" << eg.ref << " sens " << sens ;
        if (((eg[0].r - eg[1].r), (e[0].r - e[1].r)) < 0) sens = -sens;
        subdomains[i].sens = sens;
        subdomains[i].edge = &e;
        //	cout << " sens " << sens << " in geom " << eg[0].r << eg[1].r << " in mesh  " <<
        //e[0].r << e[1].r << endl; 	cout << "  v0 , v1 = " << Number(v0) << " "  << Number(v1) <<
        //endl;
        assert(t && sens);
        ...
        while (1) {
          ...
          if (ta.EdgeVertex(0) == v1) {    // ok we find the edge
            ...
            if (mark[it] >= 0) {
              ...
              if (verbosity > 10)
                cerr << " Warning: the subdomain " << i << " ref = " << subdomains[i].ref
                     << " is previouly defined with " << mark[it]
                     << " ref = " << subdomains[mark[it]].ref << " skip this definition " << endl;
              break;
            }
            if (i != inew) Exchange(subdomains[i], subdomains[inew]);
            inew++;
            Triangle *tt = t;
            Int4 kkk = 0;
            do {
            ...
            if (verbosity > 7)
              cout << " Number of triangles in each subdomain " << i << " of ref "
                   << subdomains[i].ref << " = " << kkk << endl;
            break;
          }
          ta = Previous(Adj(ta));
          if (t == (Triangle *)ta) {
            err++;
            cerr << " Error in the definition of subdomain " << i << " edge = " << Gh.Number(eg)
                 << " " << sens << endl;
            break;
          }
    ...
    if (verbosity > 4) cout << "    ";
    if (verbosity > 2)
      cout << " Number of bounded subdomains = " << NbSubDomTot << " NbOutTriangles = " << NbOutT
           << endl;
#ifdef DRAWING1
    inquire( );
#endif
  ...
  void Triangles::ReNumberingVertex(Int4 *renu) {
    ...
    for (it = 0; it < nbt; it++) triangles[it].ReNumbering(vertices, ve, renu);

    for (ie = 0; ie < nbe; ie++) edges[ie].ReNumbering(vertices, ve, renu);

    for (i = 0; i < NbVerticesOnGeomVertex; i++) {
      Vertex *v = VerticesOnGeomVertex[i].mv;
      if (v >= vertices && v < ve) VerticesOnGeomVertex[i].mv = vertices + renu[Number(v)];
    }

    for (i = 0; i < NbVerticesOnGeomEdge; i++) {
      Vertex *v = VerticesOnGeomEdge[i].mv;
      if (v >= vertices && v < ve) VerticesOnGeomEdge[i].mv = vertices + renu[Number(v)];
    }

    for (i = 0; i < NbVertexOnBThVertex; i++) {
      Vertex *v = VertexOnBThVertex[i].v;
      if (v >= vertices && v < ve) VertexOnBThVertex[i].v = vertices + renu[Number(v)];
    }

    for (i = 0; i < NbVertexOnBThEdge; i++) {
      Vertex *v = VertexOnBThEdge[i].v;
      if (v >= vertices && v < ve) VertexOnBThEdge[i].v = vertices + renu[Number(v)];
    }
  ...
  void Triangles::ReNumberingTheTriangleBySubDomain(bool justcompress) {
    ...
    for (it = 0; it < nbt; it++) renu[it] = -1;    // outside triangle
    for (i = 0; i < NbSubDomains; i++) {
    ...
    if (verbosity > 9) cout << " Number of inside triangles " << k << " nbt = " << nbt << endl;
    // take is same numbering if possible
    if (justcompress)
      for (k = 0, it = 0; it < nbt; it++)
        if (renu[it] >= 0) renu[it] = k++;
    ...
    for (it = 0; it < nbt; it++) triangles[it].ReNumbering(triangles, te, renu);
  ...
  */
  Vertex *Triangles::NearestVertex(Icoor1 i, Icoor1 j) { return quadtree->NearestVertex(i, j); }
  ...
  void Triangles::PreInit(Int4 inbvx, char *fname) {
    ...
    if (name || inbvx) {
      ...
      identity = new char[lg + strlen(buf) + strlen(countbuf) + 2 + 10 +
                          (Gh.name ? strlen(Gh.name) + 4 : 0)];
      identity[0] = 0;
      if (lg) strcat(strcat(strcat(identity, "B="), BTh.name), ", ");

      if (Gh.name) strcat(strcat(identity, "G="), Gh.name);
    ...
    //  Meshend  = vertices + nbvx;
    if (verbosity > 98)
      cout << " Triangles::PreInit() " << nbvx << " " << nbtx << " " << vertices << " " << ordre
           << " " << triangles << endl;
  }
    ...
    //  find the begin of the curve in BTh
    {
      ...
      /*
         ...
         for(int i=0;i<Gh.NbOfCurves ;i++)
           {
              cout << " Curve " << i << " begin e=" << Gh.Number(Gh.curves[i].be) << " k=" <<
         Gh.curves[i].kb
      ...
      for (int iedge = 0; iedge < BTh.nbe; iedge++) {
        ...
        for (int je = 0; je < 2; je++)    // for the 2 extremites
          if (!ei.on->Mark( ) && ei[je].on->IsRequiredVertex( )) {
            ...
            // "
            //     << " e "  << " " << Gh.Number(ei.on) << " vc " <<
            //     Gh.Number((*Gh.curves[nc].be)[Gh.curves[nc].kb]) << endl;
            if (ei.on == Gh.curves[nc].be &&
                (GeometricalVertex *)*ei[je].on ==
                  &(*Gh.curves[nc].be)[Gh.curves[nc].kb]    //  same extremity
            ) {
              // cout << " find " << endl;
      ...
      */
      ...
      for (int icurve = 0; icurve < Gh.NbOfCurves; icurve++) {
        ...
        for (int phase = 0; phase <= step; phase++) {
              ...
              for (;;) {
                ...
                if (phase) {    // computation of the new points
                  while ((i != NbCreatePointOnCurve) && sNew <= L) {
                    //    cout  << " L0= " << L0 << " L " << L << " sN="
                    //         << sNew << " LAB=" << LAB << " NBPC =" <<NbCreatePointOnCurve<< " i "
                    //         << i  << endl;
                    assert(sNew >= L0);
                    assert(LAB);
                    ...
                    VertexOnBThEdge[NbVerticesOnGeomEdge++] =
                      VertexOnEdge(A1, &eeequi, se);    // save
                    ...
                    if (verbosity > 99)
                      cout << i << "+ New P " << nbv - 1 << " " << sNew << " L0=" << L0
                           << " AB=" << LAB << " s=" << (sNew - L0) / LAB << " se= " << se
                           << " B edge " << BTh.Number(ee) << " sign = " << k1 << " " << A1->r
                           << endl;
                    ...
                    if (dp > 1.4) {
                      ...
                      cerr << " PB calcul new on cuver points trop loin l=" << dp
                           << " v=" << nbv - 1 << " " << nbv - 2 << " Lcurve = " << Lcurve << AB
                           << v0.m << v1.m << endl;
                    }
                ...
                if (verbosity > 98)
                  cout << BTh.Number(ee) << " "
                       << " on=" << *ee[k1].on << " " << ee[k1].on->IsRequiredVertex( ) << endl;
                ...
                if (!ee.adj[k1]) {
                  cerr << "Error adjacent edges " << BTh.Number(ee) << ", nbe = " << nbe
                       << " Gh.vertices " << Gh.vertices << " k1 = " << k1 << " on=" << *ee[k1].on
                       << endl;
          ...
          if (!phase) {                                           //
            ...
            if (verbosity > 5)
              cout << icurve << " NbSegOnCurve = " << NbSegOnCurve << " Lstep=" << Lstep << " "
      ...
      if (step == 0) {
        ...
        if (nbv + NbOfNewPoints > nbvx) {
          cerr << " Too many vertices on geometry " << nbv + NbOfNewPoints << " >= " << nbvx
               << endl;
          MeshError(3, this);
        }
          ...
          for (int j = 0; j < 2; j++)
            if (!ei.Mark( ) && ei[j].Required( )) {
              ...
              if (ei.Required( )) {
                ...
                for (int kstep = 0; kstep <= step;
                     kstep++) {    // begin  for ( int kstep=0;kstep<= step;kstep++)
                  ...
                  for (;;) {
                    ...
                    while (lcurve <= s && s <= lcurveb && nbv < nbvend) {
                      ...
                      if (verbosity > 999) {
                        ...
                        cout << " new edge " << nbe - 1 << " ";
                        cout << va << vb << " kk0 = " << kk0 << " " << kk1 << " ss=" << ss;
                        cout << " " << sbb << endl;
                        cout << "      " << aa << va->r << bb << vb->r
                             << " length=" << Norme2(va->r - vb->r) << endl;
                        cout << "      s " << s << " lstep= " << lstep << " ledge= " << ledge
                             << " lcurve= " << lcurve << endl;
                      }
                  ...
                  //    << lcurve << " NbEdgeCurve " << NbEdgeCurve << " "
                  //    <<NbVerticesOnGeomEdge0<<" " <<NbVerticesOnGeomEdge<<" step =" <<step<<
                  //    endl;
                }
                // end of curve --
  ...
  Edge **Triangles::MakeGeometricalEdgeToEdge( ) {
    ...
    for (i = 0; i < Gh.nbe; i++)
      if (!e[i])
        if (kk++ < 10) {
          cerr << " Bug -- the geometrical edge " << i
               << " is on no edge curve = " << Gh.edges[i].CurveNumber << " s0 "
               << Gh.Number(Gh.edges[i][0]) << " s1 " << Gh.Number(Gh.edges[i][1]) << endl;
          //	 assert( e[i]);
        }
    ...
    for (i = 0; i < nbt; i++) {
      ...
      if (pv0 && pv1 && pv2)    // a good triangles;
      {
        ...
        if (triangles[i].det <= 0 && Nberr++ < 10) {
          if (Nberr == 1) {
            if (strfrom)
              cerr << "+++ Fatal Error " << strfrom << "(SetInCoor) Error: area of triangle < 0 "
                   << endl;
            else
          ...
          cerr << Number(pv0) << " " << Number(pv1) << " " << Number(pv2) << " : ";
  ...
  void Triangles::FillHoleInMesh( ) {
    ...
    // generation of the integer coor
    {
      ...
      Int4 i;
      if (verbosity > 2)
        cout << " -- FillHoleInMesh: Number of vertices =" << nbv << " Pmin = " << pmin
             << " Pmax = " << pmax << endl;
      ...
      for (i = 0; i < nbt; i++)
        for (int j = 0; j < 3; j++) {
          ...
          if (st[k] == -1)
            ...
            cerr << " The edge (" << Number(triangles[i][VerticesOfTriangularEdge[j][0]]) << " , "
                 << Number(triangles[i][VerticesOfTriangularEdge[j][1]])
                 << " ) is in more than 2 triangles " << k << endl;
            cerr << " Edge " << j << " Of Triangle " << i << endl;
            cerr << " Edge " << (-st[k] + 2) % 3 << " Of Triangle " << (-st[k] + 2) / 3 << endl;
            cerr << " Edge "
                 << triangles[(-st[k] + 2) / 3].NuEdgeTriangleAdj((int)((-st[k] + 2) % 3))
                 << " Of Triangle "
                 << Number(triangles[(-st[k] + 2) / 3].TriangleAdj((int)((-st[k] + 2) % 3)))
                 << endl;
            MeshError(9999, this);
          }
        }
      ...
      if (verbosity > 9) cout << " Nbtriafillhole triafillhole*" << triafillhole << endl;
  ...
  Triangles::Triangles(Triangles &Th, Geometry *pGh, Triangles *pBth,
    ...
    if (NbVerticesOnGeomVertex) VerticesOnGeomVertex = new VertexOnGeom[NbVerticesOnGeomVertex];
    NbVerticesOnGeomEdge = Th.NbVerticesOnGeomEdge;
    if (NbVerticesOnGeomEdge) VerticesOnGeomEdge = new VertexOnGeom[NbVerticesOnGeomEdge];
    if (&BTh == &Th.BTh)    // same back ground
    {
      BTh.NbRef++;
      NbVertexOnBThVertex = Th.NbVertexOnBThVertex;
      if (NbVertexOnBThVertex) VertexOnBThVertex = new VertexOnVertex[NbVertexOnBThVertex];
      NbVertexOnBThEdge = Th.NbVertexOnBThEdge;
      if (NbVertexOnBThEdge) VertexOnBThEdge = new VertexOnEdge[NbVertexOnBThEdge];
    ...
    if (nbe) edges = new Edge[nbe];
    if (NbSubDomains) subdomains = new SubDomain[NbSubDomains];
    pmin = Th.pmin;
    pmax = Th.pmax;
    coefIcoor = Th.coefIcoor;
    for (i = 0; i < nbt; i++) triangles[i].Set(Th.triangles[i], Th, *this);
    for (i = 0; i < nbe; i++) edges[i].Set(Th, i, *this);
    for (i = 0; i < nbv; i++) vertices[i].Set(Th.vertices[i], Th, *this);
    for (i = 0; i < NbSubDomains; i++) subdomains[i].Set(Th, i, *this);
    for (i = 0; i < NbVerticesOnGeomVertex; i++)
  ...
  jc ; while ( ttc->swap(jc,koption)) { NbSwap++,assert(k++<20000); ttc =  t->at[j]; jc =
  NextEdge[t->aa[j]&3]; cout << "\n\t s  " <<  *ttc << " " << jc << endl;
      }
      cout << endl;
      t = ttc;
      j = NextEdge[jc];
  ...
  */
  ...
  void Triangles::SmoothingVertex(int nbiter, Real8 omega) {
    ...
    for (k = 0; k < nbiter; k++) {
      ...
      if (verbosity > 3)
        cout << "    Move max = " << sqrt(delta) << " iteration = " << k
             << " Nb of Swap = " << NbSwap << endl;
    }
  ...
  void Triangles::MakeQuadTree( ) {
    if (verbosity > 8) cout << "      MakeQuadTree" << endl;
    if (!quadtree) quadtree = new QuadTree(this);
    ...
    */
    ...
    for (int it = 0; it < nbt; it++)
      if (triangles[it].link) {
        ...
        Marea += betaK;
        // cout << betaK << " " << area3 << " " << beta << " " << beta0 << " " << area3*3*3*3
        // <<endl;
        beta = min(beta, betaK);
  ...
  int Triangles::UnCrack( ) {
    assert(NbCrackedEdges == 0 || NbCrackedVertices > 0);
    for (int i = 0; i < NbCrackedEdges; i++) CrackedEdges[i].UnCrack( );
    return NbCrackedEdges;
  }
  ...
  int Triangles::CrackMesh( ) {
    ...
    if (nbcrakev)
      for (int iec = 0; iec < NbCrackedEdges; iec++) CrackedEdges[iec].Set( );
    ...
    if (nbcrakev && verbosity > 1)
      cout << " Number of craked vertices = " << nbcrakev << " Number of created vertices "
           << nbnewv << endl;
    ...
    if (nbnewv) {    //
      ...
      for (i = 0; i < NbVerticesOnGeomVertex; i++) vog[i] = VerticesOnGeomVertex[i];
    ...
    if (vlast >= vend) {
      cerr << " Too few vertices. In order to crack the mesh, we need " << nbv << " vertices "
           << endl;
      MeshError(555, this);
    }
  ...
  Triangles::Triangles(const Triangles &Tho, const int *flag, const int *bb)
    : Gh(*(new Geometry( ))), BTh(*this) {    // truncature
    //
    ...
    for (i = 0; i < Tho.nbv; i++) kk[i] = -1;
    for (i = 0; i < Tho.nbv; i++) refv[i] = 0;
    int nbNewBedge = 0;
    //  int nbOldBedge =0;
    for (i = 0; i < Tho.nbt; i++)
      if (reft[i] >= 0 && flag[i]) {
    ...
    for (i = 0; i < Tho.nbv; i++)
      if (kk[i] >= 0) {
        ...
        if (!vertices[nbv].ref( )) vertices[nbv].ReferenceNumber = refv[i];
        nbv++;
      }
    assert(inbvx == nbv);
    for (i = 0; i < Tho.nbt; i++)
      if (reft[i] >= 0 && flag[i]) {
        ...
        // cout <<i<< " F" <<  flag[i] << " T " << nbt << "   = " <<  kk[i0] << " " << kk[i1] << " "
        // << kk[i2] ; cout << " OT  " <<  i0 << " "  << i1 << " " << i2  << " " << reft[i] << endl;
  ...
  Triangle *Triangles::FindTriangleContening(const I2 &B, Icoor2 dete[3],
                                             Triangle *tstart) const {    // in: B
    // out: t
    // out : dete[3]
    ...
    if (tstart)
      ...
      if (!a || !a->t) {
        if (a) {
          cerr << " Warning: problem TriangleConteningTheVertex vertex number=" << Number(a)
               << endl;
    ...
    while (t->det > 0) {
      ...
      if ((t1.det( ) < 0) && (k == 2)) t1 = t->Adj(jj = ii[1]);
      t = t1;
