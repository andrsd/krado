...
#ifndef NDEBUG
inline void
MyAssert(int i, char * ex, char * file, long line)
{
    if (i) {
...
Int4
FindTriangle(Triangles & Th, Real8 x, Real8 y, double * a, int & inside)
{
    CurrentTh = &Th;
    assert(&Th);
...
TriangleAdjacent
CloseBoundaryEdge(I2 A, Triangle * t, double & a, double & b)
{
    //
    //  cout << " - ";
...
TriangleAdjacent
Triangle::FindBoundaryEdge(int i) const
{
    ...
    int k = 0;
#endif
    int j = EdgesVertexTriangle[i][0], jc;
    int exterieur = !link;
...
TriangleAdjacent
CloseBoundaryEdgeV2(I2 C, Triangle * t, double & a, double & b)
{
...
Metric
Triangles::MetricAt(const R2 & A) const
...
void
ListofIntersectionTriangles::SplitEdge(const Triangles & Bh, const R2 & A, const R2 & B, int nbegin)
{ //  SplitEdge
    ...
    if (t->det < 0) { // outside departure
        while (t->det < 0) { // intersection boundary edge and a,b,
            ...
            //         cout << Bh.Number(tbegin) << " " << Bh.Number(t) << " i= " << i <<" j= " <<
            //         j << " k=" << k
            //       	   << " deti= " << deti << " detj= " << detj
...
int
ListofIntersectionTriangles::NewItem(Triangle * tt, Real8 d0, Real8 d1, Real8 d2)
{
    int n;
    R2 x(0, 0);
    if (d0)
...
int
ListofIntersectionTriangles::NewItem(R2 A, const Metric & mm)
{
    int n;
...
#ifdef DEBUG1
        if (SHOW)
            cout << "SHOW       ++ NewItem A" << Size << A << endl;
#endif
        n = Size++;
    }
    else
...
Real8
ListofIntersectionTriangles::Length()
{
    //  cout << " n= " << Size << ":" ;
    assert(Size > 0);
    ...
    for (jj = 0, ii = 1; ii < Size; jj = ii++) {
        ...
        lIntTria[ii].s = s;
        if (ii == EndSeg)
            SegI->lEnd = s, SegI++, EndSeg = SegI->last, SegI->lBegin = s;
...
Int4
ListofIntersectionTriangles::NewPoints(Vertex * vertices, Int4 & nbv, Int4 nbvx)
{
...
{ // l'arete ta coupe l'arete pva pvb
  ...
  // attention la routine echange pva et pvb
    ...
    if (CurrentTh &&
        ...
        cout << " detvasasb =" << detvasasb << "detvbsasb = " << detvbsasb << " " << pva << " "
             << pvb << " " << CurrentTh << endl;
    #ifdef DRAWING1
        reffecran();
        CurrentTh->Draw();
...
#endif
    if (ToSwap)
        NbSwap++, bamg::swap(t1, a1, t2, a2, &s1, &s2, ndet1, ndet2);
...
int
ForceEdge(Vertex & a, Vertex & b, TriangleAdjacent & taret)
{
#ifndef NDEBUG
restart: // for debug
#endif
        ...
        if ((det1 < 0) && (det2 > 0)) {
            ...
            while ((ks = SwapForForcingEdge(va, vb, tc, detss, det1, det2, NbSwap)))
                if (l++ > 10000000) {
                    cerr << " Loop in forcing Edge AB "
                         << "\n vertex A " << a << "\n vertex B " << b << "\n nb de swap " << NbSwap
                         << "\n nb of try swap too big = " << l << " greater than " << 1000000
                         << endl;

                    if (CurrentTh)
                        cerr << " vertex number " << CurrentTh->Number(a) << " "
                             << CurrentTh->Number(b) << endl;
#ifdef DEBUG
                    ForDebugging = 1;
#endif
#ifdef DRAWING1
            ...
            if (((aa == &a) && (bb == &b)) || ((bb == &a) && (aa == &b))) {
                ...
                /*	  cerr << "Fatal Error  boundary is crossing  ";
                          if(CurrentTh)
                          {
                              cerr << " edge:  [" << CurrentTh->Number(a) << ", " <<
                   CurrentTh->Number(b)
                   <<  " ] and [ ";               cerr    << CurrentTh->Number(aa) << " " <<
                   CurrentTh->Number(bb) << " ] " << endl;
                          }
                          MeshError(991);
                */
            }
...
int
Triangle::swap(Int2 a, int koption)
{
#ifdef DEBUG
    if (a & 4)
        return 0; // arete lock
    ...
    if (sa == 0) { // les deux triangles sont frontieres
        ...
        if (verbosity > 99999)
            cout << " Triangle::swap int " << OnSwap << " " << det1 << " " << det2 << " "
                 << detMinNew << endl;
            ...
            if (!OnSwap) {
                ...
                while (1)
                    if (kopt) {
                        ...
                        Icoor2 xb1 = sb->i.x - s1->i.x, x21 = s2->i.x - s1->i.x,
                               yb1 = sb->i.y - s1->i.y, y21 = s2->i.y - s1->i.y,
                               xba = sb->i.x - sa->i.x, x2a = s2->i.x - sa->i.x,
                               yba = sb->i.y - sa->i.y, y2a = s2->i.y - sa->i.y;
                        double cosb12 = double(xb1 * x21 + yb1 * y21),
                               cosba2 = double(xba * x2a + yba * y2a), sinb12 = double(det2),
                        ...
                        if (verbosity > 99999)
                            cout << " Triangle::swap " << OnSwap << " "
                                 << ((double) cosb12 * (double) sinba2) << " "
                                 << ((double) cosba2 * (double) sinb12) << " "
                                 << ((double) cosba2 * (double) sinb12) -
                                        ((double) cosb12 * (double) sinba2)
                                 << endl;
                        //  	 if(CurrentTh)
                        //  	   cout << "swap " << CurrentTh->Number(sa) << " " <<
                        //  CurrentTh->Number(sb)
                        //  << " " ; 	 cout <<  cosb12 << " " <<  sinba2 << " "  <<  cosba2 << " "
                        //  << sinb12
                        //  	      << " Onswap = " <<  OnSwap << endl;
                        break;
                    }
...
void
Triangles::Add(Vertex & s, Triangle * t, Icoor2 * det3)
{
    // -------------------------------------------
    //             s2
    //                                            !
    ...
    if (nbd0 > 0) // point s on a egde or on a vertex
    {
        if (nbd0 == 1) {
            ...
            cout << " ta = " << ta << "ta->det =" << ((Triangle *) ta)->det << " " << t->det
                 << endl;
#endif
    ...
    *tt[1] = *tt[2] = *t;
    ...
    if (izerodet >= 0) {
        ...
        if (!rswap) {
            cout << " Problem swap: the point s is on an edge => swap " << iedge << " "
                 << *tt[izerodet] << endl;
#ifdef DRAWING
...
Int4
Triangles::SplitInternalEdgeWithBorderVertices()
{
    Int4 NbSplitEdge = 0;
    SetVertexFieldOn();
    Int4 it;
...
Int4
Triangles::InsertNewPoints(Int4 nbvold, Int4 & NbTSwap)
{
...
#ifdef DEBUG
    {
        ...
        for (i = 0; i < nbt; i++)
            if (triangles[i].link) {
                double dd = Det(triangles[i][1].r - triangles[i][0].r,
                                triangles[i][2].r - triangles[i][0].r);
                if (dd <= 0) {
                    NbErr++;
                ...
                for (i = 0; i < nbt; i++)
                    if (triangles[i].link) {
                        ...
                        if (dd <= 0) {
                            ...
                            cerr << Number(triangles[i][0]) << " " << Number(triangles[i][1]) << " "
                                 << Number(triangles[i][2]) << endl;
                            cerr << triangles[i][0].r << triangles[i][1].r << triangles[i][2].r
                                 << endl;
                            cerr << triangles[i][0].i << triangles[i][1].i << triangles[i][2].i
                                 << endl;
                        }
                    }
            }
...
void
Triangles::NewPoints(Triangles & Bh, int KeepBackVertex)
{ // Triangles::NewPoints
...
void
Triangles::NewPointsOld(Triangles & Bh)
{ // Triangles::NewPointsOld
        ...
        for (t = &triangles[i]; i < nbt; t = &triangles[i = next_t],
            ...
            for (j = 0; j < 3; j++) { // for each edge
                ...
                if (first_np_or_next_t[k] > 0) { // this edge is done before
                    ...
                    assert((*t)(VerticesOfTriangularEdge[j][0]) ==
                           (*ta)(VerticesOfTriangularEdge[kk][1]));
                    assert((*t)(VerticesOfTriangularEdge[j][1]) ==
                           (*ta)(VerticesOfTriangularEdge[kk][0]));
                    Int4 kolor = 3 * k + kk;
                    // ColorEdge[j] = kolor;
                    Int4 kkk = 1;
                    step[j] = -1; // other sens
                ...
                for (Int4 i0 = BeginNewPoint[j0]; i0 <= EndNewPoint[j0]; i0++) {
                    ...
                    while (j0 != (j1 = NextEdge[j1])) { // loop on the 2 other edge
                        ...
                        if (sss * ((cii0 = vi0.m(D, (R2) vertices[ii0])) - c0) > 0)
                            ...
                            if (ii1 != ii0)
                                stack[kstack++] = ii1; // cout << " add1 " << ii1;
                        }
#ifdef DEBUG2
                        cout << "ii1 = " << ii1 << " ii0 = " << ii0 << endl;
                        cout << "   cccc = " << cii0 << " " << ciii << " " << cii1 << " sss=" << sss
                             << endl;
#endif
                        if (kstack > 5) // bug ?
                            cout << "NewPoints: bug????? " << kstack << " stack " << stack[kstack]
                                 << endl;
                    }
                    ...
                    while ((i1 = stack[kstack++]) >= 0) { // the two parameter is i0 and i1
                        ...
                        if (d01 < seuil) {
                            if (i1 < nbvold) {
                                ...
                                for (ip0 = i0; i0 != (ipp0 = vertices[ip0].ReferenceNumber);
                                     ip0 = ipp0)
                                    kk0++;
                                for (ip1 = i1; i1 != (ipp1 = vertices[ip1].ReferenceNumber);
                                     ip1 = ipp1)
                                    kk1++;
                                ...
                                Exchange(vertices[i0].ReferenceNumber,
                                         vertices[i1].ReferenceNumber);
                                // the new coordinate
                                R2 C //= vertices[i0] ;
    ...
    for (int it = 0; it < nbt; it++)
        if (triangles[it].det == 0)
            if (nberr++ < 10)
                cerr << "Bug Triangle nul" << it << triangles[it] << endl;
    if (nberr)
        MeshError(992, this);
#endif
...
Triangles::Insert()
{
    ...
    if (verbosity > 3)
        cout << " NbSwap of insertion " << NbSwap << " NbSwap/Nbv " << (float) NbSwap / (float) nbv
             << " NbUnSwap " << NbUnSwap << " Nb UnSwap/Nbv " << (float) NbUnSwap / (float) nbv
             << endl;
    NbUnSwap = 0;
    // construction d'un ordre aleatoire
...
void
Triangles::FindSubDomain(int OutSide = 0)
{
    // #define DRAWING1
    ...
    if (OutSide || !Gh.subdomains || !Gh.NbSubDomains) { // No geom sub domain
        ...
        if (OutSide) {
                ...
                for (it = 0; it < nbt && nbk; it++)
                    for (int na = 0; na < 3 && nbk; na++) {
                        ...
                        if (kr != kl) {
                            ...
                            if (kl >= 0 && subdomains[kl].ref < 0 && kr >= 0 &&
                                subdomains[kr].ref >= 0)
                                nbk--, subdomains[kr].ref = subdomains[kl].ref - 1;
                            if (kr >= 0 && subdomains[kr].ref < 0 && kl >= 0 &&
                                subdomains[kl].ref >= 0)
            ...
            if (verbosity > 4)
                cout << " Number of removed subdomains (OutSideMesh) = " << NbSubDomains - j
                     << endl;
            NbSubDomains = j;
        }
        ...
        for (Int4 i = 0; i < NbSubDomains; i++) {
            ...
            // e[0].r << e[1].r << endl; 	cout << "  v0 , v1 = " << Number(v0) << " "  <<
            // Number(v1) << endl;
            assert(t && sens);
            ...
            while (1) {
                ...
                if (ta.EdgeVertex(0) == v1) { // ok we find the edge
                    ...
                    if (mark[it] >= 0) {
                        if (verbosity > 10)
                            cerr << " Warning: the subdomain " << i
                                 << " ref = " << subdomains[i].ref << " is previouly defined with "
                                 << mark[it] << " ref = " << subdomains[mark[it]].ref
...
void
Triangles::ReNumberingVertex(Int4 * renu)
{
...
void
Triangles::ReNumberingTheTriangleBySubDomain(bool justcompress)
{
...
Int4
Triangles::ConsRefTriangle(Int4 * reft) const
{
    assert(reft);
    Triangle *t0, *t;
    Int4 k = 0, num;
...
*/
Vertex *
Triangles::NearestVertex(Icoor1 i, Icoor1 j)
{
    return quadtree->NearestVertex(i, j);
}

void
Triangles::PreInit(Int4 inbvx, char * fname)
{
    mysrand(19999999);
    OnDisk = 0;
    NbRef = 0;
...
void
Triangles::GeomToTriangles1(Int4 inbvx, int KeepBackVertices)
{
    // #define DRAWING1
    ...
    if (NbVerticesOnGeomVertex > nbvx) {
        cerr << " Too many vertices on geometry " << NbVerticesOnGeomVertex << " > " << nbvx
             << endl;
        MeshError(1, this);
    }
    assert(vertices);
    ...
    //  find the begin of the curve in BTh
    {
        ...
        /*
           ...
           Gh.curves[i].kb
                     << "  end e= " << Gh.Number(Gh.curves[i].ee) << " k=" << Gh.curves[i].ke <<
           endl;
             }*/
        for (int i = 0; i < Gh.NbOfCurves; i++) {
            bcurve[i] = -1;
        ...
        for (int iedge = 0; iedge < BTh.nbe; iedge++) {
            ...
            for (int je = 0; je < 2; je++) // for the 2 extremites
                if (!ei.on->Mark() && ei[je].on->IsRequiredVertex()) {
                    ...
                    // cout << "curve " <<  nc << " v " << Gh.Number((GeometricalVertex *)
                    // *ei[je].on) << "
                    // "
        ...
        */
        ...
        for (int icurve = 0; icurve < Gh.NbOfCurves; icurve++) {
            ...
            if (!Gh.curves[icurve].master)
                continue; // we skip all equi curve
#ifndef NDEBUG
            Edge & ei = BTh.edges[iedge];
#endif
                        ...
                        for (;;) {
                            ...
                            if (phase) { // computation of the new points
                                while ((i != NbCreatePointOnCurve) && sNew <= L) {
                                    ...
                                    // assert(VerticesOnGeomEdge && NbVerticesOnGeomEdge <
                                    // NbVerticesOnGeomEdgex); new vertex on edge
                                    A1 = vertices + nbv++;
...
#ifndef NDEBUG
                                    se = abscisseInterpole(v0.m,
                                                           v1.m,
                                                           AB,
                                                           se); // because code \ref(xxx)
#else
                                    ...
                                    e->v[1] = A1;
                                    if (verbosity > 99)
                                        cout << i << "+ New P " << nbv - 1 << " " << sNew
                                             << " L0=" << L0 << " AB=" << LAB
                                             << " s=" << (sNew - L0) / LAB << " se= " << se
                                             << " B edge " << BTh.Number(ee) << " sign = " << k1
                                             << " " << A1->r << endl;
                                    ...
                                    if (dp > 1.4) {
                                        ...
                                        cerr << " AB=" << LAB << " s=" << (sNew - L0) / LAB
                                             << " se= ";
                                        cerr << se << " B edge " << BTh.Number(ee)
                                             << " signe = " << k1 << endl;
                                        cerr << " PB calcul new on cuver points trop loin l=" << dp
                                             << " v=" << nbv - 1 << " " << nbv - 2
                                             << " Lcurve = " << Lcurve << AB << v0.m << v1.m
                                             << endl;
                                    }
                            ...
                            if (verbosity > 98)
                                cout << BTh.Number(ee) << " "
                                     << " on=" << *ee[k1].on << " " << ee[k1].on->IsRequiredVertex()
                                     << endl;
                            ...
                            if (!ee.adj[k1]) {
                                cerr << "Error adjacent edges " << BTh.Number(ee)
                                     << ", nbe = " << nbe << " Gh.vertices " << Gh.vertices
...
void
Triangles::GeomToTriangles0(Int4 inbvx)
{
    Gh.NbRef++; // add a ref to GH
    ...
    if (verbosity > 10)
        cout << " GeomToTriangles0: init nbv : " << nbv << endl;
    //  assert( Gh.nbv < nbvx);
                            ...
                            for (int kstep = 0; kstep <= step;
                                ...
                                for (;;) {
                                    ...
                                    while (lcurve <= s && s <= lcurveb && nbv < nbvend) {
                                        ...
                                        vb->r = e->F(abcisse);
                                        VerticesOnGeomEdge[NbVerticesOnGeomEdge++] =
                                        ...
                                        nbe++;
                                        //  remove DEBUG print to find a bug in
                                        //  examples++-tutorial/periodic4.edp
                                        // under windows
                                        //  after this the bug  disappear (version 3.30 on freefem++
                                        //  23/04/2014  FH problem optimisation, bug in compiler,
                                        //  bug in freefem++

                                        if (verbosity > 999) {
                                            cout << " new points " << nbv - 1 << " " << vb->r;
                                            cout << " new edge " << nbe - 1 << " ";
                                            cout << va << vb << " kk0 = " << kk0 << " " << kk1
                                                 << " ss=" << ss;
                                            cout << " " << sbb << endl;
                                            cout << "      " << aa << va->r << bb << vb->r
                                                 << " length=" << Norme2(va->r - vb->r) << endl;
                                            cout << "      s " << s << " lstep= " << lstep
                                                 << " ledge= " << ledge << " lcurve= " << lcurve
                                                 << endl;
                                        }
                                ...
                                //    <<NbVerticesOnGeomEdge0<<" " <<NbVerticesOnGeomEdge<<" step ="
                                //    <<step<< endl;
                            }
                            // end of curve --
...
Edge **
Triangles::MakeGeometricalEdgeToEdge()
{
    assert(Gh.nbe);
...
void
Triangles::SetIntCoor(const char * strfrom)
{
    pmin = vertices[0].r;
    pmax = vertices[0].r;
    ...
    for (i = 0; i < nbt; i++) {
        ...
        if (pv0 && pv1 && pv2) // a good triangles;
        {
            ...
            if (triangles[i].det <= 0 && Nberr++ < 10) {
                if (Nberr == 1) {
                    if (strfrom)
                        cerr << "+++ Fatal Error " << strfrom
                             << "(SetInCoor) Error: area of triangle < 0 " << endl;
                    else
                        cerr << "+++ Fatal Error Triangle (in SetInCoor) area of triangle < 0"
                             << endl;
                }
    ...
    // generation of the integer coor
    {
        ...
        for (i = 0; i < nbt; i++)
            for (int j = 0; j < 3; j++) {
                ...
                if (st[k] == -1)
                    ...
                    cerr << " Edge " << (-st[k] + 2) % 3 << " Of Triangle " << (-st[k] + 2) / 3
                         << endl;
                    cerr << " Edge "
                         << triangles[(-st[k] + 2) / 3].NuEdgeTriangleAdj((int) ((-st[k] + 2) % 3))
                         << " Of Triangle "
                         << Number(
                                triangles[(-st[k] + 2) / 3].TriangleAdj((int) ((-st[k] + 2) % 3)))
                         << endl;
                    MeshError(9999, this);
                }
            }
        ...
        if (k != 0) {
            if (verbosity > 20) {
                ...
                for (int i = 0; i < nbe; i++)
                    cout << " Edge " << i << " : " << Number(edges[i][0]) << " "
                         << Number(edges[i][1]) << " " << edges[i].ref << endl;
            }
...
void
Triangles::SmoothingVertex(int nbiter, Real8 omega)
{
...
void
Triangles::ShowRegulaty() const // Add FH avril 2007
{
...
Triangle *
Triangles::FindTriangleContening(const I2 & B, Icoor2 dete[3], Triangle * tstart) const
{ // in: B
    // out: t
    // out : dete[3]
