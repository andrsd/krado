...
namespace bamg {
  ...
  MetricAnIso::MetricAnIso(const Real8 a[3], const MetricAnIso m0, const MetricAnIso m1,
  ...
  MatVVP2x2::MatVVP2x2(const MetricAnIso M) {
    ...
    } else {    //    ---  construction  de 2 vecteur dans (Im ( A - D(i) Id) ortogonal
      delta = sqrt(delta);
  ...
  {
    if (verbosity > 1)
      cout << "  -- IntersectGeomMetric geometric err=" << err << (iso ? " iso " : " aniso ")
           << endl;
  ...
  */
  void Triangles::BoundAnisotropy(Real8 anisomax, Real8 hminaniso) {
    ...
    if (verbosity > 1) cout << "  -- BoundAnisotropy by  " << anisomax << endl;
    Real8 h1 = 1.e30, h2 = 1e-30, rx = 0;
    Real8 coef = 1. / (anisomax * anisomax);
    Real8 hn1 = 1.e30, hn2 = 1e-30, rnx = 1.e-30;
    for (Int4 i = 0; i < nbv; i++) {
  ...
  void Triangles::IntersectConsMetric(const double *s, const Int4 nbsol, const int *typsols,
                                      ...
                                      const int DoNormalisation, const double power,
                                      const int choice) {    //  the array of solution s is store
    ...
    if (verbosity > 1) {
      cout << "  -- Construction of Metric: Nb of field. " << n << " nbt = " << nbt
           << " nbv= " << nbv << " coef = " << coef << endl
           << "     hmin = " << hmin << " hmax=" << hmax << " anisomax = " << anisomax
           << " Nb Jacobi " << NbJacobi << " Power = " << power;
      if (RelativeMetric)
      ...
      if (triangles[i].link)    // the real triangles
      {
        ...
        for (int j = 0; j < 3; j++) {
          ...
          if (!ta || !ta->link)    // no adj triangle => edge on boundary
            ...
            OnBoundary[Number(t[VerticesOfTriangularEdge[j][1]])] = 1, nbb++;
        }
    ...
    for (Int4 nusol = 0; nusol < nbsol; nusol++) {    // for all Solution
      ...
      Real8 hn1 = 1.e30, hn2 = 1e-30, rnx = 1.e-30;
      int nbfield =
        typsols ? (typsols[nusol] < 4 ? sizeoftype[typsols[nusol]] : typsols[nusol] + 1) : 1;
      if (nbfield == 1)
        ...
        for (iv = 0, k = 0; iv < nbv; iv++, k += n) {
          double v = 0;
          for (int i = 0; i < nbfield; i++) v += ss[k + i] * ss[k + i];
          v = sqrt(v);
          smin = Min(smin, v);
          smax = Max(smax, v);
        }
      ...
      if (verbosity > 2)
        cout << "    Solution " << nusol << " Min = " << smin << " Max = " << smax
             << " Delta =" << sdelta << " cnorm = " << cnorm << " Nb of fields =" << nbfield
             << endl;
      ...
      for (Int4 nufield = 0; nufield < nbfield; nufield++, ss++) {
        ...
        for (i = 0; i < nbt; i++)
          if (triangles[i].link) {    // for real all triangles
            // coor of 3 vertices
            R2 A = triangles[i][0];
            R2 B = triangles[i][1];
            R2 C = triangles[i][2];
            ...
            if (choice) {
              ...
              //	det33=1;
              // verif
              //	cout << " " << (taa[0][0]*cBC +  taa[1][0]*cCA + taa[2][0] * cAB)/det33 << "
              //== " << bb[0] ; 	cout << " " << (taa[0][1]*cBC +  taa[1][1]*cCA + taa[2][1] *
              //cAB)/det33 << " == " << bb[1]; 	cout << " " << (taa[0][2]*cBC +  taa[1][2]*cCA +
              //taa[2][2] * cAB)/det33 << " == " << bb[2]
              //	     << "  -- " ;
              ...
              Real8 coef2 = 2 * coef;
              //	cout << " H = " << Hxx << " " << Hyy << " " <<  Hxy/2 << " coef2 = " <<
              //coef2 << endl;
              Hxx *= coef2;
              Hyy *= coef2;
              Hxy *= coef2;
              // cout << i  << " H = " << 3*Hxx/dd << " " << 3*Hyy/dd << " " <<  3*Hxy/(dd*2) << "
              // nbb = " << nbb << endl;
              if (nbb == 0) {
                dxdx[iA] += Hxx;
                dydy[iA] += Hyy;
        ...
        for (int xy = 0; xy < 3; xy++) {
          ...
          for (int ijacobi = 0; ijacobi < Max(NbJacobi, 2); ijacobi++) {
            ...
            for (iv = 0; iv < nbv; iv++)
              if (ijacobi < NbJacobi || OnBoundary[iv]) dd[iv] = workV[iv] / (Mmass[iv] * 6);
          }
        }
        ...
        for (iv = 0, k = 0; iv < nbv; iv++, k += n) {    // for all vertices
          ...
          if (RelativeMetric) {    //   compute the norm of the solution
            ...
            for (int ifield = 0; ifield < nbfield; ifield++, sfk++) xx += *sfk * *sfk;
            xx = sqrt(xx);
        ...
        if (verbosity > 2) {
          ...
          cout << "              before bounding :  Hmin = " << sqrt(1 / h2)
               << " Hmax = " << sqrt(1 / h1) << " factor of anisotropy max  = " << sqrt(rx) << endl;
          cout << "              after  bounding :  Hmin = " << sqrt(1 / hn2)
               << " Hmax = " << sqrt(1 / hn1) << " factor of anisotropy max  = " << sqrt(rnx)
               << endl;
        }
  ...
  void Triangles::ReadMetric(const char *fmetrix, const Real8 hmin1 = 1.0e-30,
                             const Real8 hmax1 = 1.0e30, const Real8 coef = 1) {
    Real8 hmin = Max(hmin1, MinimalHmin( ));
    Real8 hmax = Min(hmax1, MaximalHmax( ));
    ...
    f_metrix >> k >> j;
    if (verbosity > 1)
      cout << " metrix: open " << fmetrix << ", le coef = " << coef << ", hmin = " << hmin
           << ", hmax = " << hmax << ((j == 1) ? " Iso " : " AnIso ") << endl;

    if (k != nbv || !(j == 1 || j == 3)) {
      cerr << " Error Pb metrix " << k << " <> " << nbv << " or  1 or 3 <> " << j << endl;
      MeshError(1002);
    }
  ...
  void Triangles::WriteMetric(ostream &f, int iso) {
    if (iso) {
      ...
      f << nbv << " " << 3 << endl;
      for (Int4 iv = 0; iv < nbv; iv++)
        f << vertices[iv].m.a11 << " " << vertices[iv].m.a21 << " " << vertices[iv].m.a22 << endl;
    }
  }
  ...
  void Triangles::SmoothMetric(Real8 raisonmax) {
    ...
    for (i = 0; i < nbv - 1; i++) first_np_or_next_t0[i] = i + 1;
    first_np_or_next_t0[nbv - 1] = -1;    // end;
    for (i = 0; i < nbv; i++) first_np_or_next_t1[i] = -1;
    kk = 0;
    while (Head0 >= 0 && kk++ < 100) {
      kch = 0;
      for (i = Head0; i >= 0; i = first_np_or_next_t0[ip = i],
          first_np_or_next_t0[ip] =
                                -1) {    //  pour tous les triangles autour du sommet s
                                         // 	cout << kk << " i = " << i << " " << ip << endl;
    ...
    if (verbosity > 2 && verbosity < 5) cout << "    Nb of Loop " << kch << endl;
    delete[] first_np_or_next_t0;
    delete[] first_np_or_next_t1;
  }

  void Geometry::ReadMetric(const char *fmetrix, Real8 hmin = 1.0e-30, Real8 hmax = 1.0e30,
                            Real8 coef = 1) {
    ...
    f_metrix >> k >> j;
    if (verbosity > 1)
      cout << "  -- ReadMetric  " << fmetrix << ",  coef = " << coef << ", hmin = " << hmin
           << ", hmax = " << hmax << ((j == 1) ? " Iso " : " AnIso ") << endl;
